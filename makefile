...
DEBUG ?= 0
ifeq ($(DEBUG), 1)
	CFLAGS+=-O0 -DDEBUGi -fstrict-aliasing -msse4 -std=c++17
	...
	CFLAGS+=-DNDEBUG -Ofast -flto -msse4 -fstrict-aliasing  -std=c++17
	CFLAGS2+=-DNDEBUG -Ofast -flto -msse4 -fstrict-aliasing -std=c++17
	WARNS=-Wfatal-errors
endif
